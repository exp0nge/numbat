...
| Cursor | `~/.cursor/projects/<hash>/agent-transcripts/**/*.jsonl` | `~/.cursor/hooks.json` | yes — `preToolUse` | `state.vscdb` is deferred. numbat uses Cursor's native `permission:"deny"` response. The default user hook covers local Agent sessions; Cloud and Tab limits are below. |
...
| Cursor | numbat uses the unfiltered generic `preToolUse`, `postToolUse`, and `postToolUseFailure` stream so specialized shell, MCP, and file callbacks do not double-report the same action. Enforcement applies only to `preToolUse`; `subagentStart` is monitoring-only because current Cursor builds invoke it but ignore its deny response. For local sessions, all matching Enterprise, Team, trusted Project, and User hooks run. Cloud agents load repository Project hooks and, on Enterprise plans, Team and Enterprise hooks; they do not load `~/.cursor/hooks.json`, may perform initial read-only exploratory turns before hooks begin, and do not emit `sessionStart` or `sessionEnd`. Cursor Tab uses separate `beforeTabFileRead` / `afterTabFileEdit` events that numbat does not install. |
